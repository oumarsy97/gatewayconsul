...
import { sampleWithFullData, sampleWithNewData, sampleWithPartialData, sampleWithRequiredData } from '../commande.test-samples';

import { CommandeService, RestCommande } from './commande.service';

const requireRestSample: RestCommande = {
...
describe('Commande Service', () => {
  ...
  describe('Service methods', () => {
    ...
    it('should create a Commande', () => {
      const commande = { ...sampleWithNewData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.create(commande).subscribe(resp => (expectedResult = resp.body));
    ...
    it('should update a Commande', () => {
      const commande = { ...sampleWithRequiredData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.update(commande).subscribe(resp => (expectedResult = resp.body));
    ...
    describe('addCommandeToCollectionIfMissing', () => {
      it('should add a Commande to an empty array', () => {
        const commande: ICommande = sampleWithRequiredData;
      ...
      it("should add a Commande to an array that doesn't contain it", () => {
        const commande: ICommande = sampleWithRequiredData;
        const commandeCollection: ICommande[] = [sampleWithPartialData];
        expectedResult = service.addCommandeToCollectionIfMissing(commandeCollection, commande);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(commande);
      });
      ...
      it('should add only unique Commande to an array', () => {
        const commandeArray: ICommande[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const commandeCollection: ICommande[] = [sampleWithRequiredData];
      ...
      it('should accept varargs', () => {
        const commande: ICommande = sampleWithRequiredData;
        const commande2: ICommande = sampleWithPartialData;
    ...
    describe('compareCommande', () => {
      it('Should return true if both entities are null', () => {
        ...
        const compareResult = service.compareCommande(entity1, entity2);

        expect(compareResult).toEqual(true);
      });

      it('Should return false if one entity is null', () => {
        const entity1 = { id: 10216 };
        const entity2 = null;

        const compareResult1 = service.compareCommande(entity1, entity2);
        const compareResult2 = service.compareCommande(entity2, entity1);
